...
with open('Diverse_ligands_len.txt', 'r') as f:
    ct = f.read()
...
df_PS_maxmin = pd.read_csv('Diverse_ligands_PS_maxmin.csv',index_col=0)
...
for i in range(0,length_Decoy):
    MW_MCS_Decoy.append(Normalization(MW_Decoy[i],MW_Ligand_Max,MW_Ligand_Min))
    NR_MCS_Decoy.append(Normalization(NR_Decoy[i],NR_Ligand_Max,NR_Ligand_Min))
    HD_MCS_Decoy.append(Normalization(HD_Decoy[i],HD_Ligand_Max,HD_Ligand_Min))
    HA_MCS_Decoy.append(Normalization(HA_Decoy[i],HA_Ligand_Max,HA_Ligand_Min))
    FC_MCS_Decoy.append(Normalization(FC_Decoy[i],FC_Ligand_Max,FC_Ligand_Min))
    LogP_MCS_Decoy.append(Normalization(LogP_Decoy[i],LogP_Ligand_Max,LogP_Ligand_Min))
...
df_Decoy.to_csv('Final_decoys.csv', index=False)
