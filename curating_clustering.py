...
def main(input_f, output_f):
    ...
    sim_matrix = np.array([DataStructs.BulkTanimotoSimilarity(
        MACCS_fp[i], MACCS_fp[:n_mols_], returnDistance=True) for i in range(n_mols_)])

    clustering = AgglomerativeClustering(
        n_clusters=39, linkage='ward', affinity='euclidean')
